...
res.redirect(`https://discord.com/oauth2/authorize?client_id=${clientID}&redirect_uri=swordie.miniatuurfiguur.repl.co/thanks&response_type=code&scope=bot&permissions=490073207`)
});
...
member.send(new MessageEmbed().setColor('GREEN').setFooter(`Powered by `).setTitle(`Application #${ticketID}`).setDescription(`Hey ${member.user.username}! Your form was Submitted and ready to be judged.\n\n**Form ID**: \`${ticketID}\`\n**Time:** ${moment(new Date()).format("dddd, MMMM Do YYYY HH:mm:ss")}`)).catch(()=>{});
...
channel.send(embed)
 ...
 app.get("/redeem", checkAuth, (req, res) => {
    ...
    app.get("/dashboard", checkAuth, (req, res) => {
      ...
      const server = client.guilds.cache.get('963486925988180008');
      ...
      if (Object.prototype.hasOwnProperty.call(data, "sendEmbed")) {
        ...
        if (ticketSettings.ticketToggle == "true") {
          ...
          if (ticketSettings.ticketType == "reaction" && ticketSettings.ticketCustom == "false") {
            ...
            if (storedSettings.isPremium == "false") {
              ticketSettings.ticketFooter = "Powered by Dash.bot.fun";
            } else {
            ...
            let reactionFooter = "Powered by Dash.bot.fun";



            let footer = "Powered by Dash.bot.fun";
            ...
            if (storedSettings.isPremium == "false") {
              ticketEmbed.setFooter(`Powered by Dash.bot.fun`)
            } else {
...
if(config.datadogApiKey){
  metrics.init({ apiKey: jsonconfig.datadogApiKey, host: '', prefix: '.' });
